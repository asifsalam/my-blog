...
<div class="content">
	...
	<div class="main-content">
		...
		<p>
			...
			government communcation between far flung locations required secrecy and was often encrypted.
			The ancient Spartans (~7 BC) used the scytale transposition cipher to encrypt messages during
			military campaigns, and Romans used the "substitution cipher". Documents in India recommended
			the use of a cipher to conceal the content of communications between lovers. This secrecy was
			even more important for those plotting against this state, since discovery would, well, lead
			to some rather unpleasant consequences.
		</p>
		...
		<p>
			...
			usual sequence. An alternative would be to shift the letters by an agreed number. Or create a
			key using random placement of the letters. There are other, more sophisticated techniques, but
			these simpler ones are convenient for illustrating the concept.
		</p>
		<p>
		...
		<p>
			...
			in the cipher. But with a little care, this type of encryption was unbreakable because of the
		...
		<p>
			...
			these messages. Al-Kindi invented what is now called frequency analysis, a statistical method
			for breaking this type of encryption. In his book "A Manuscript on Deciphering Cryptographic
			Messages" (841 AD), Al-Kindi describes the method:
		</p>
		<p class="quote-block">
			<span
				class="ciphetext"
				...
				‘third’, and so on, until we account for all the different letters in the plaintext sample.”</span
			>
			<span
			...
			>
				...
				correct in long enough messages to correspond to all places of frequent and rare occurrences
				so that if some letters are [too] few in one segment of the message, they will be [too] many
				in others. But if the cryptogram is too short, equivalence does not apply, letter ranks are
				not correct and [consequently] a second trick should be used to recover letters. Such a
				trick is qualitative which is ... [here al-Kindi wrote in detail on possible letter
				combinations in a language, like 'al' in Arabic...etc.] (1)."
			</span>
		</p>
		...
		<p>
			This message was has been encrypted by a substitution cipher. As a codebreaker, your job is to
			decode the ciphertext. You have trained on the Al-Kindi texts. You know that the letter
			frequencies are only a starting point. You will have to use your statistical knowledge of the
			English language, its common letter combinations and sequences. Your toolkit consists of the
			following:
		</p>
		<ol>
		...
		<p>
			<strong
				>Symbols positions in the lower row can be swapped by clicking on the first symbol to select
				it and then the second symbol to swap the two. Symbols that have been swapped are coloured
				green.</strong
			>
		</p>
		<p>
			The key is applied to the ciphertext to decode it. This decoding is updated as the symbols are
			swapped in the key table above.
		</p>
		...
		<div class="references">
			...
			<ol>
				...
				<li>
					Naveed, M., Kamara, S., and Wright, C.V., (2015). Inference Attacks on Property-Preserving
					Encrypted Databases. In <i
